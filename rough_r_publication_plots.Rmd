...
cairo_pdf("graphs/ie_map.pdf", width=9, height=3.5)
ggplot(world) +
  #geom_polygon(fill = "lightgray", colour = "white", size = 0.2) +
  geom_sf(size=0.2) +
  coord_sf(xlim = c(-25, 60), ylim = c(30, 70), expand=T, datum=NA) +
  geom_point(data = trs_IE_prop, aes(x = longitude, y = latitude, color = r_prop, group = NA),
             alpha = 0.9, position = position_jitter(0.1), size = 3) +
  facet_grid(~rough.f) +
  scale_colour_gradient(name="proportion\nof /r/",
                        low = other_col, high=r_col) + # labels=c("0% /r/", "contains /r/")) +
  labs(title = "/r/ in rough and smooth words in Indo-European languages",
...
cairo_pdf("graphs/ie_model_preds.pdf", width=4.5, height=3.5)
ggplot(data = trs_IE_prop, aes(x = rough.f, y = r_prop*100)) +
  geom_point(data=trs_IE_prop, 
             aes(color = r_prop, size = n), alpha = 0.5,
  ...
  labs(title = "Proportions per language and\nmodel predictions (Indo-European)",
...
ggplot(world) +
  ...
  geom_point(data = xling_single, aes(x = Longitude, y = Latitude, color = r, group = NA),
