...
auto_add <- read_csv('../raw_data/a4_cross_linguistic/autotyp_additions.csv')
...
chir <- rename(chir,
	...
	select(Language, Meaning, Form, ISO_code, Glotto_code)
...
reflex <- rename(reflex,
	...
	Meaning = TRA,
	POS = CGR) %>%
	select(Language, Meaning, Form, POS) %>%
...
IDS <- rename(IDS,
	...
	select(Language, Meaning, Form, Glotto_code)
...
google <- rename(trs,
	...
	Meaning = eng_orig,
	ISO_code = iso639.3) %>%
	select(Language, Meaning, Form, ISO_code)
...
(Do we need glottolog info? Isn't Autotyp enough?)

```{r}
# ## Add Glottolog info, Reflex:
# 
# reflex$Glotto_code <- glotto[match(reflex$ISO_code, glotto$iso639P3code), ]$id
# 
# ## Hand-add the missing information:
# 
# reflex[reflex$Language == 'Kaado', ]$Glotto_code <- 'kaad1238'
# reflex[reflex$Language == 'Yoómbe', ]$Glotto_code <- 'yoom1238'
# reflex[reflex$Language == 'Kanuri', ]$Glotto_code <- 'kanu1279'
# reflex[reflex$Language == 'Nembe', ]$Glotto_code <- 'nemb1238'
# 
# ## Add Glottolog info, Google:
# # (when a particular variety was chosen to stand in for the family was chosen this
# # does not affect the results as the family id's would be the same):
# 
# google$Glotto_code <- glotto[match(google$ISO_code, glotto$iso639P3code), ]$id
# google[google$Language == 'Azeerbaijani', ]$Glotto_code <- 'nort2697'
# google[google$Language == 'Croatian', ]$Glotto_code <- 'croa1245'
# google[google$Language == 'Estonian', ]$Glotto_code <- 'esto1258'
# google[google$Language == 'Gujarati', ]$Glotto_code <- 'guja1252'
# google[google$Language == 'Malagasy', ]$Glotto_code <- 'anta1255'
# google[google$Language == 'Pashto', ]$Glotto_code <- 'cent1973'
# google[google$Language == 'Serbian', ]$Glotto_code <- 'serb1264'
```

Re-ordering / filtering columns in data sets for easier merging.

```{r}
IDS <- select(IDS, Language,
	ISO_code, Phoible_code, Meaning, Form, Trill)
reflex <- select(reflex, Language,
	ISO_code, Phoible_code, Meaning, Form, Trill)
chir <- select(chir, Language,
	ISO_code, Phoible_code, Meaning, Form, Trill)
google <- select(google, Language,
	ISO_code, Phoible_code, Meaning, Form, Trill)
```
...
We now merge the data sets and add a column that identifies the original data set.
...
Language family and areal info are merged from Autotyp and roughness predictors are added from eng.
...
# 2) if all rhotics are of type X, it returns that type
# 3) if there's no rhotic, it returns "no rhotic" (this is actually achieved by 2 above)
# 4) otherwise returns the label mixed (meaning that there is more than one rhotic type)
check_rhotics <- function (x) {
  if (length(x)==1 && is.na(x)) {
    return (x)
  } else if (mean(x==x[1])==1) {
    return(x[1])
  } else {
    return ("mixed")
  }
...
# adding R type data to main data set  
xling$R_type <- all_types[match(xling$Phoible_code, all_matches)]
```
...
xling[xling$Language == 'Ese Ejja', ]$Trill <- 'yes'
...
We exclude languages that have no rhotics. Note that this also includes some languages where the phonetic value of rhotics is unusual, e.g. uvular fricatives (Breton, Hebrew) or retroflex fricatives (Hanoi Vietnamese).

```{r}
xling$R_type[is.na(xling$R_type)] <- "no info"
xling <- filter(xling, R_type!="no rhotic")
```
