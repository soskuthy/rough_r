...
y <- logistic_summary(eng_brm_all.l_mod, eng, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
...
y <- logistic_summary(eng_brm_pie.l_mod, eng, outcome="/l/", roughpred="rough", pp_over_zero=F)
```  
...
trs_IE %>%
  count(language, rough, l) %>%
  group_by(language, rough) %>%
  mutate(prop=n / sum(n)) %>%
ggplot(aes(x=rough, y=prop, fill=l)) +
  facet_wrap(~language) +
  geom_bar(stat="identity") +
  scale_fill_manual(values=c("deepskyblue4","firebrick3"))
```
...
# fewer /l/'s in rough words per language
fewer_ls_in_rough <- trs_IE %>%
  count(language, rough, l) %>%
  group_by(language, rough) %>%
  mutate(prop=n / sum(n)) %>%
  ungroup() %>%
  filter(l) %>%
  complete(language, rough, fill=list(l=T, n=0, prop=0)) %>%
  group_by(language) %>%
  summarise(fewer_ls_in_rough = (prop[rough] < prop[!rough])) %>%
  ungroup()
cat("The proportion of /l/'s is lower in rough words in", 
    sum(fewer_ls_in_rough$fewer_ls_in_rough), "out of",
    nrow(fewer_ls_in_rough), "languages, which is",
    round(sum(fewer_ls_in_rough$fewer_ls_in_rough)*100 / nrow(fewer_ls_in_rough),2),
    "per cent\n")
...
1a) predicted likelihood of R for smooth vs. rough with 95% CIs
  (smooth:  [,]; rough: [,])
1b) estimated difference with 95% CIs
  ( [,])
2a) predicted likelihood of L for smooth vs. rough with 95% CIs
  (smooth: [,]; rough [,])
2b) estimated difference with 95% CIs
  ( [,])

```{r}
x <- logistic_summary(trs_IE_brm_r_mod, eng, outcome="/r/", roughpred="rough", pp_over_zero=T)
y <- logistic_summary(trs_IE_brm_l_mod, eng, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
...
# ranger output 
# hun_rf_mod_all2 <- cforest(hun_rf_all_formula, data = hun_aggr, control=cforest_control(mtry = round(sqrt(length(hun_rf_all_preds))), ntree = 5000))
...
preds <- beta_summary(hun_brm_beta_mod, hun_aggr, rpred="all.r", lpred="all.l", binary_pred=F, printPlease=T)
```
...
y <- logistic_summary(hun_brm_l_mod, eng, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
...
set.seed(314)
hun_ie_brm_l_mod <- brm(all.l ~ rough,
                        data=hun_ie,
                        prior=hun_brm_priors,
                        family="bernoulli",
                        refresh=0)
summary(hun_ie_brm_l_mod)
```
...
y <- logistic_summary(hun_ie_brm_l_mod, eng, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
...
- We'll analyse languages with trills vs. those with no trills separately (as we only have a prediction for Ls with trills). We use a simple strategy here: the column /r/ codes for the presence of any kind of rhotic, but languages where Trill=="yes" only have trills. Therefore, we simply limit the range of languages to those with Trill=="yes" (and, for the complementary analysis, Trill=="no").
- We fit three different models:
  - roughness rating as the outcome, r / l as predictors, beta regression (full data set); random effects by Family, Area, Language
  - r / l as outcome, binary rough as predictor (full data set); random effects by Family, Area, Language
  - r / l as outcome, binary rough as predictor (limited specifically to the words rough / smooth); random effects by Family, Area
...
```{r}
preds <- beta_summary(xling_brm_beta_mod, dat=xling_trill, rpred="r", lpred="l", binary_pred=T, printPlease=T)
```
...
y <- logistic_summary(xling_brm_logistic_mod_l, dat=xling_trill, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
...
y <- logistic_summary(xling_brm_rs_logistic_mod_l, dat=xling_trill, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
...
```{r}
preds <- beta_summary(xling_nt_brm_beta_mod, dat=xling_trill, rpred="r", lpred="l", binary_pred=T, printPlease=T)
```
...
y <- logistic_summary(xling_nt_brm_logistic_mod_l, dat=xling_trill, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
...
y <- logistic_summary(xling_nt_brm_rs_logistic_mod_l, dat=xling_trill, outcome="/l/", roughpred="rough", pp_over_zero=F)
```
