...
### 1.5 Modelling hardness instead of roughness

Hardness and roughness are related. Therefore (following a suggestion from a reviewer), we now investigate whether the same pattern is present for roughness.

We start by looking at some descriptive stats. The distribution of hardness ratings -- like those for roughness -- is heavily bimodal.

```{r}
eng$hard <- eng$Hard.M >= 0

# density plot
ggplot(eng, aes(x=Hard.M)) +
  geom_density(fill="deepskyblue4")
```

Here are the proportions of /r/ in hard vs. soft words, along with Cohen's D for the difference in hardness ratings across words with / without /r/.
...
# Cohen's D
cohen.d(Hard.M ~ all.r, filter(eng, !is.na(hard)), pooled=T)

# difference in means:
lm(Hard.M ~ all.r, filter(eng, !is.na(hard)))
```

This difference is somewhat smaller than that seen for roughness: 25% (as opposed to 33%) when looking at the proportion of r in hard vs. soft; and a small effect according to Cohen's D. The difference in mean ratings between r-ful vs. r-less is about 1.5.

We now run a logistic regression model similar to the one for roughness.

```{r, warning=F, message=F}
eng_brm_all.x_priors <- c(set_prior("student_t(5,0,2.5)", class = "b"),
                          set_prior("student_t(5,0,2.5)", class = "Intercept"))
set.seed(314)
eng_brm_all.r_hard_mod <- brm(all.r ~ hard,
                         data=filter(eng, !is.na(hard)),
                         prior=eng_brm_all.x_priors,
                         family="bernoulli",
                         refresh=0)
summary(eng_brm_all.r_hard_mod)
```

Model predictions. Note that while the effect is in the right direction, the credible interval around the difference term does, in fact, include 0. (With almost 95% of samples over zero, though. So it's sort of borderline - but certainly not by any means strong evidence.)

```{r}
x <- logistic_summary_hard(eng_brm_all.r_hard_mod, eng, outcome="/r/", hardpred="hard", pp_over_zero=T)
```
...
We now ask: looking at the exact same words (i.e. same origin, e.g. borrowing, from PIE, etc.), but focusing on other common sounds, do we observe the same degree of stability in the association of roughness with those sounds across languages? Looking at it from the perspective of PIE: do random associations between sounds other than /r/ and roughness remain as consistent across the daughter languages as that between /r/ and roughness? To test this, we fit logistic regression models with rough as the outcome and the presence of different sounds as the predictor + random slopes over "rough" by language, and we check the random slope sds, which tell us about how much the languages vary in the strength of the rough-[given consonant] effect.
...
The latest version of our cross-linguistic data set includes 394 languages from 84 language families (counting isolates as families) from 19 Autotyp areas. Without Indo-European, there are 347 languages and 83 families from 19 Autotyp areas. We exclude Indo-European as we've already analysed these languages above.
...
Our main analysis focuses specifically on the words rough and smooth. We report numbers for these in the paper. 657 words, 329 languages, 83 families, 19 areas.
...
Of these, trilled only: 379 words, 178 languages, 43 families, 18 areas.
...
And non-trilled only: 278 words, 151 languages, 60 families, 16 areas.
...
Limiting the data set to languages with trills only leaves 850 data points from 185 languages representing 43 families and 18 areas.
...
Further limiting the data set to words for rough/smooth only leaves 379 data points from 178 languages representing 43 families and 18 areas. 
...
Model predictions as usual. Note that the /r/ pattern does not seem strong when our "rough" predictor lumps words of different levels of roughness / smoothness together and does not take into account the actual amount of roughness / smoothness for specific words.
...
Model predictions as usual. When focusing on these two extreme words only, the pattern is there.
...
Limiting the data set to languages without trills (or where it's not clear whether the language has a trill) leaves 397 data points from 159 languages representing 61 families and 16 areas.
...
Further limiting the data set to words for rough/smooth only leaves 270 data points from 147 languages representing 59 families and 16 areas.
...
xling_brm_omnibus_mod_r <- brm(r ~ rough * Trill +
                          ...
                          (1 + rough * Trill | Area),
                        data=xling_rs,
                        prior=xling_brm_omnibus_rs_intr_priors,
                        family="bernoulli",
                       control=list(adapt_delta=0.99))
